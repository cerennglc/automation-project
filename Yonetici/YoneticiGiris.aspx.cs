            ...
            using (var db = new OtomasyonDBEntities())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var sorguTC = db.KisiTablosu.Where(x => x.TC == kullaniciTC && x.Sifre == kullaniciSifre && x.YoneticiMi == true).Count();

                        if (sorguTC > 0)
                        {
                            Session["TC"] = kullaniciTC;
                            Response.Redirect("Kisiler.aspx"); //Giriş başarılı ve Response.redirect ile anasayfaya yönlendiriyoruz.
                        }
                        else
                        {
                            lblSonuc.Visible = true;
